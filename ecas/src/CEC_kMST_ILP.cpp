...
Stats CEC_kMST_ILP::solve()
{
	...
	try {
		...
		/*CutCallback* usercb = new CutCallback( env, model_type, epOpt, instance, x, z, y );
		CutCallback* lazycb = new CutCallback( env, model_type, epOpt, instance, x, z, y );
		...
		stringstream ss;
		ss << k;
		ofstream solution(("res/CEC_" + instance.instance_file.replace(0,5,"").replace(3,8,"") + "_" + ss.str() + ".sol").c_str());
